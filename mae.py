...
class MAE(nn.Module):
    ...
    def forward(self, img):
        ...
        # original image
        original_image = self.reconstruct_image(patches=patches, model_input=img, patch_size=self.patch_size)
        masked_image = self.reconstruct_image(patches=patches, model_input=img, masked_indices=masked_indices, patch_size=self.patch_size)
        reconstructed_image = self.reconstruct_image(patches=patches, model_input=img, masked_indices=masked_indices, pred_pixel_values=pred_pixel_values, patch_size=self.patch_size)
